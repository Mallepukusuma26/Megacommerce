"""
MegaCommerce Enterprise Domain Module: ml_analytics -> supplier_performance_index
Description: On-time in-full supplier score
"""
import math
import time
import datetime
...
class SupplierPerformanceIndexStatus:
    PENDING = "PENDING"
...
class SupplierPerformanceIndexException(Exception):
    """Custom exception for SupplierPerformanceIndex domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class SupplierPerformanceIndexConfig:
    """Configuration container for SupplierPerformanceIndex."""
...
class SupplierPerformanceIndexEngine:
    """Enterprise Business Logic Engine for SupplierPerformanceIndex."""
    def __init__(self, config: Optional[SupplierPerformanceIndexConfig] = None):
        self.config = config or SupplierPerformanceIndexConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['On-Time Rate', 'Defect Rate', 'Price Variance', 'Supplier Rank (A/B/C)']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise SupplierPerformanceIndexException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise SupplierPerformanceIndexException(f'Record ID {record_id} not found')
...
def execute_supplier_performance_index_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing supplier_performance_index pipeline."""
    engine = SupplierPerformanceIndexEngine()
    processed_results = []
