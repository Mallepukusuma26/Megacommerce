"""
MegaCommerce Enterprise Domain Module: ml_analytics -> supplier_performance_index_extended
Description: Extended enterprise capabilities for On-time in-full supplier score
"""
import math
import time
import datetime
...
class SupplierPerformanceIndexExtendedStatus:
    PENDING = "PENDING"
...
class SupplierPerformanceIndexExtendedException(Exception):
    """Custom exception for SupplierPerformanceIndexExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class SupplierPerformanceIndexExtendedConfig:
    """Configuration container for SupplierPerformanceIndexExtended."""
...
class SupplierPerformanceIndexExtendedEngine:
    """Enterprise Business Logic Engine for SupplierPerformanceIndexExtended."""
    def __init__(self, config: Optional[SupplierPerformanceIndexExtendedConfig] = None):
        self.config = config or SupplierPerformanceIndexExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_On-Time Rate', 'Ext_Defect Rate', 'Ext_Price Variance', 'Ext_Supplier Rank (A/B/C)', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise SupplierPerformanceIndexExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise SupplierPerformanceIndexExtendedException(f'Record ID {record_id} not found')
...
def execute_supplier_performance_index_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing supplier_performance_index_extended pipeline."""
    engine = SupplierPerformanceIndexExtendedEngine()
    processed_results = []
