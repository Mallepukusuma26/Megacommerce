"""
MegaCommerce Enterprise Domain Module: payment_sim -> fraud_velocity_checker_extended
Description: Extended enterprise capabilities for Card velocity anomaly rule
"""
import math
import time
import datetime
...
class FraudVelocityCheckerExtendedStatus:
    PENDING = "PENDING"
...
class FraudVelocityCheckerExtendedException(Exception):
    """Custom exception for FraudVelocityCheckerExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class FraudVelocityCheckerExtendedConfig:
    """Configuration container for FraudVelocityCheckerExtended."""
...
class FraudVelocityCheckerExtendedEngine:
    """Enterprise Business Logic Engine for FraudVelocityCheckerExtended."""
    def __init__(self, config: Optional[FraudVelocityCheckerExtendedConfig] = None):
        self.config = config or FraudVelocityCheckerExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_CVV Attack Guard', 'Ext_Rapid Card Swipe', 'Ext_IP Swap Detection', 'Ext_High Amount Trigger', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise FraudVelocityCheckerExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise FraudVelocityCheckerExtendedException(f'Record ID {record_id} not found')
...
def execute_fraud_velocity_checker_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing fraud_velocity_checker_extended pipeline."""
    engine = FraudVelocityCheckerExtendedEngine()
    processed_results = []
