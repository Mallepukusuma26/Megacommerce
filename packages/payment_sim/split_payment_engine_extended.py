"""
MegaCommerce Enterprise Domain Module: payment_sim -> split_payment_engine_extended
Description: Extended enterprise capabilities for Card + GiftCard split checkout
"""
import math
import time
import datetime
...
class SplitPaymentEngineExtendedStatus:
    PENDING = "PENDING"
...
class SplitPaymentEngineExtendedException(Exception):
    """Custom exception for SplitPaymentEngineExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class SplitPaymentEngineExtendedConfig:
    """Configuration container for SplitPaymentEngineExtended."""
...
class SplitPaymentEngineExtendedEngine:
    """Enterprise Business Logic Engine for SplitPaymentEngineExtended."""
    def __init__(self, config: Optional[SplitPaymentEngineExtendedConfig] = None):
        self.config = config or SplitPaymentEngineExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_Card + Wallet', 'Ext_Card + GiftCard', 'Ext_UPI + Wallet', 'Ext_Multi Card', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise SplitPaymentEngineExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise SplitPaymentEngineExtendedException(f'Record ID {record_id} not found')
...
def execute_split_payment_engine_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing split_payment_engine_extended pipeline."""
    engine = SplitPaymentEngineExtendedEngine()
    processed_results = []
