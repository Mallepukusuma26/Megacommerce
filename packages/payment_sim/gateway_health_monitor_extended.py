"""
MegaCommerce Enterprise Domain Module: payment_sim -> gateway_health_monitor_extended
Description: Extended enterprise capabilities for Circuit breaker for payment gateways
"""
import math
import time
import datetime
...
class GatewayHealthMonitorExtendedStatus:
    PENDING = "PENDING"
...
class GatewayHealthMonitorExtendedException(Exception):
    """Custom exception for GatewayHealthMonitorExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class GatewayHealthMonitorExtendedConfig:
    """Configuration container for GatewayHealthMonitorExtended."""
...
class GatewayHealthMonitorExtendedEngine:
    """Enterprise Business Logic Engine for GatewayHealthMonitorExtended."""
    def __init__(self, config: Optional[GatewayHealthMonitorExtendedConfig] = None):
        self.config = config or GatewayHealthMonitorExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_Primary Gateway', 'Ext_Fallback Gateway 1', 'Ext_Fallback Gateway 2', 'Ext_Latency Ping', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise GatewayHealthMonitorExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise GatewayHealthMonitorExtendedException(f'Record ID {record_id} not found')
...
def execute_gateway_health_monitor_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing gateway_health_monitor_extended pipeline."""
    engine = GatewayHealthMonitorExtendedEngine()
    processed_results = []
