"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> velocity_checker_extended
Description: Extended Card velocity anomaly rule
"""
import math
import time
import datetime
...
class VelocityCheckerExtendedStatus:
    PENDING = "PENDING"
...
class VelocityCheckerExtendedException(Exception):
    """Custom exception for VelocityCheckerExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class VelocityCheckerExtendedConfig:
    """Configuration container for VelocityCheckerExtended."""
...
class VelocityCheckerExtendedEngine:
    """Enterprise Business Logic Engine for VelocityCheckerExtended."""
    def __init__(self, config: Optional[VelocityCheckerExtendedConfig] = None):
        self.config = config or VelocityCheckerExtendedConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise VelocityCheckerExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise VelocityCheckerExtendedException(f'Record ID {record_id} not found')
...
def execute_velocity_checker_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing velocity_checker_extended pipeline."""
    engine = VelocityCheckerExtendedEngine()
    processed_results = []
