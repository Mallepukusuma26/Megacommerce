"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> upi_simulator_core
...
class UPISimulatorCoreException(Exception):
    """Custom exception for UPISimulatorCore domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class UPISimulatorCoreConfig:
    """Configuration container for UPISimulatorCore."""
...
class UPISimulatorCoreEngine:
    """Enterprise Business Logic Engine for UPISimulatorCore."""
    def __init__(self, config: Optional[UPISimulatorCoreConfig] = None):
        self.config = config or UPISimulatorCoreConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise UPISimulatorCoreException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise UPISimulatorCoreException(f'Record ID {record_id} not found')
...
def execute_upi_simulator_core_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing upi_simulator_core pipeline."""
    engine = UPISimulatorCoreEngine()
    processed_results = []
