"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> rate_limiter_core
...
class RateLimiterCoreException(Exception):
    """Custom exception for RateLimiterCore domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class RateLimiterCoreConfig:
    """Configuration container for RateLimiterCore."""
...
class RateLimiterCoreEngine:
    """Enterprise Business Logic Engine for RateLimiterCore."""
    def __init__(self, config: Optional[RateLimiterCoreConfig] = None):
        self.config = config or RateLimiterCoreConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise RateLimiterCoreException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise RateLimiterCoreException(f'Record ID {record_id} not found')
...
def execute_rate_limiter_core_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing rate_limiter_core pipeline."""
    engine = RateLimiterCoreEngine()
    processed_results = []
