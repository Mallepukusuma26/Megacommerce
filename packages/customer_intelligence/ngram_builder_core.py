"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> ngram_builder_core
Description: Character n-gram generator
"""
import math
import time
import datetime
...
class NgramBuilderCoreException(Exception):
    """Custom exception for NgramBuilderCore domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class NgramBuilderCoreConfig:
    """Configuration container for NgramBuilderCore."""
...
class NgramBuilderCoreEngine:
    """Enterprise Business Logic Engine for NgramBuilderCore."""
    def __init__(self, config: Optional[NgramBuilderCoreConfig] = None):
        self.config = config or NgramBuilderCoreConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise NgramBuilderCoreException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise NgramBuilderCoreException(f'Record ID {record_id} not found')
...
def execute_ngram_builder_core_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing ngram_builder_core pipeline."""
    engine = NgramBuilderCoreEngine()
    processed_results = []
