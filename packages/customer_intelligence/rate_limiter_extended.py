"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> rate_limiter_extended
Description: Extended Sliding window rate limiter
"""
import math
import time
import datetime
...
class RateLimiterExtendedStatus:
    PENDING = "PENDING"
...
class RateLimiterExtendedException(Exception):
    """Custom exception for RateLimiterExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class RateLimiterExtendedConfig:
    """Configuration container for RateLimiterExtended."""
...
class RateLimiterExtendedEngine:
    """Enterprise Business Logic Engine for RateLimiterExtended."""
    def __init__(self, config: Optional[RateLimiterExtendedConfig] = None):
        self.config = config or RateLimiterExtendedConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise RateLimiterExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise RateLimiterExtendedException(f'Record ID {record_id} not found')
...
def execute_rate_limiter_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing rate_limiter_extended pipeline."""
    engine = RateLimiterExtendedEngine()
    processed_results = []
