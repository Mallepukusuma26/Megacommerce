"""
MegaCommerce Enterprise Domain Module: customer_intelligence -> rfm_analyzer_core
...
class RFMAnalyzerCoreException(Exception):
    """Custom exception for RFMAnalyzerCore domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class RFMAnalyzerCoreConfig:
    """Configuration container for RFMAnalyzerCore."""
...
class RFMAnalyzerCoreEngine:
    """Enterprise Business Logic Engine for RFMAnalyzerCore."""
    def __init__(self, config: Optional[RFMAnalyzerCoreConfig] = None):
        self.config = config or RFMAnalyzerCoreConfig()
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise RFMAnalyzerCoreException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise RFMAnalyzerCoreException(f'Record ID {record_id} not found')
...
def execute_rfm_analyzer_core_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing rfm_analyzer_core pipeline."""
    engine = RFMAnalyzerCoreEngine()
    processed_results = []
