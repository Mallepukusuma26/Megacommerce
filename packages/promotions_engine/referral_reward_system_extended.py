"""
MegaCommerce Enterprise Domain Module: promotions_engine -> referral_reward_system_extended
Description: Extended enterprise capabilities for Double-sided referral code
"""
import math
import time
import datetime
...
class ReferralRewardSystemExtendedStatus:
    PENDING = "PENDING"
...
class ReferralRewardSystemExtendedException(Exception):
    """Custom exception for ReferralRewardSystemExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class ReferralRewardSystemExtendedConfig:
    """Configuration container for ReferralRewardSystemExtended."""
...
class ReferralRewardSystemExtendedEngine:
    """Enterprise Business Logic Engine for ReferralRewardSystemExtended."""
    def __init__(self, config: Optional[ReferralRewardSystemExtendedConfig] = None):
        self.config = config or ReferralRewardSystemExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_Referrer $10 Credit', 'Ext_Referee $10 Discount', 'Ext_Min Spend $50', 'Ext_Fraud Self-Referral Guard', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise ReferralRewardSystemExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise ReferralRewardSystemExtendedException(f'Record ID {record_id} not found')
...
def execute_referral_reward_system_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing referral_reward_system_extended pipeline."""
    engine = ReferralRewardSystemExtendedEngine()
    processed_results = []
