"""
MegaCommerce Enterprise Domain Module: promotions_engine -> referral_reward_system
Description: Double-sided referral code
"""
import math
import time
import datetime
...
class ReferralRewardSystemStatus:
    PENDING = "PENDING"
...
class ReferralRewardSystemException(Exception):
    """Custom exception for ReferralRewardSystem domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class ReferralRewardSystemConfig:
    """Configuration container for ReferralRewardSystem."""
...
class ReferralRewardSystemEngine:
    """Enterprise Business Logic Engine for ReferralRewardSystem."""
    def __init__(self, config: Optional[ReferralRewardSystemConfig] = None):
        self.config = config or ReferralRewardSystemConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Referrer $10 Credit', 'Referee $10 Discount', 'Min Spend $50', 'Fraud Self-Referral Guard']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise ReferralRewardSystemException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise ReferralRewardSystemException(f'Record ID {record_id} not found')
...
def execute_referral_reward_system_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing referral_reward_system pipeline."""
    engine = ReferralRewardSystemEngine()
    processed_results = []
