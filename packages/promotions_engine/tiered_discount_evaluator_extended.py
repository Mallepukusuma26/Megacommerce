"""
MegaCommerce Enterprise Domain Module: promotions_engine -> tiered_discount_evaluator_extended
Description: Extended enterprise capabilities for Spend $X get Y% off rules
"""
import math
import time
import datetime
...
class TieredDiscountEvaluatorExtendedStatus:
    PENDING = "PENDING"
...
class TieredDiscountEvaluatorExtendedException(Exception):
    """Custom exception for TieredDiscountEvaluatorExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class TieredDiscountEvaluatorExtendedConfig:
    """Configuration container for TieredDiscountEvaluatorExtended."""
...
class TieredDiscountEvaluatorExtendedEngine:
    """Enterprise Business Logic Engine for TieredDiscountEvaluatorExtended."""
    def __init__(self, config: Optional[TieredDiscountEvaluatorExtendedConfig] = None):
        self.config = config or TieredDiscountEvaluatorExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_Spend $50 Get $10', 'Ext_Spend $100 Get $25', 'Ext_Spend $200 Get $60', 'Ext_Max Discount Cap', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise TieredDiscountEvaluatorExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise TieredDiscountEvaluatorExtendedException(f'Record ID {record_id} not found')
...
def execute_tiered_discount_evaluator_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing tiered_discount_evaluator_extended pipeline."""
    engine = TieredDiscountEvaluatorExtendedEngine()
    processed_results = []
