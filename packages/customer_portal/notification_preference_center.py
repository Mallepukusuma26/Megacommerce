"""
MegaCommerce Enterprise Domain Module: customer_portal -> notification_preference_center
...
class NotificationPreferenceCenterStatus:
    PENDING = "PENDING"
...
class NotificationPreferenceCenterException(Exception):
    """Custom exception for NotificationPreferenceCenter domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class NotificationPreferenceCenterConfig:
    """Configuration container for NotificationPreferenceCenter."""
...
class NotificationPreferenceCenterEngine:
    """Enterprise Business Logic Engine for NotificationPreferenceCenter."""
    def __init__(self, config: Optional[NotificationPreferenceCenterConfig] = None):
        self.config = config or NotificationPreferenceCenterConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Order Status Email', 'Promotional SMS', 'Price Drop Push', 'Weekly Digest']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise NotificationPreferenceCenterException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise NotificationPreferenceCenterException(f'Record ID {record_id} not found')
...
def execute_notification_preference_center_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing notification_preference_center pipeline."""
    engine = NotificationPreferenceCenterEngine()
    processed_results = []
