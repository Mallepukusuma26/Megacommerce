"""
MegaCommerce Enterprise Domain Module: commerce_core -> order_state_machine_extended
Description: Extended enterprise capabilities for FSM for order status transitions
"""
import math
import time
import datetime
...
class OrderStateMachineExtendedStatus:
    PENDING = "PENDING"
...
class OrderStateMachineExtendedException(Exception):
    """Custom exception for OrderStateMachineExtended domain operations."""
    def __init__(self, message: str, code: int = 400):
        super().__init__(message)
        self.message = message
        self.code = code

class OrderStateMachineExtendedConfig:
    """Configuration container for OrderStateMachineExtended."""
...
class OrderStateMachineExtendedEngine:
    """Enterprise Business Logic Engine for OrderStateMachineExtended."""
    def __init__(self, config: Optional[OrderStateMachineExtendedConfig] = None):
        self.config = config or OrderStateMachineExtendedConfig()
    ...
    def _initialize_defaults(self) -> None:
        for idx, item in enumerate(['Ext_Created -> Paid', 'Ext_Paid -> Processing', 'Ext_Processing -> Shipped', 'Ext_Shipped -> Delivered', 'Special Variant A', 'Special Variant B']):
    ...
    def register_entry(self, name: str, weight: float = 1.0, priority: int = 1, metadata: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        ...
        if not name or len(name.strip()) < 2:
            raise OrderStateMachineExtendedException('Invalid item name provided to register_entry')
    ...
    def process_evaluation(self, record_id: str, threshold: float = 10.0) -> Dict[str, Any]:
        ...
        if record_id not in self.records:
            raise OrderStateMachineExtendedException(f'Record ID {record_id} not found')
...
def execute_order_state_machine_extended_pipeline(input_data: List[Dict[str, Any]]) -> Dict[str, Any]:
    """Functional entrypoint for executing order_state_machine_extended pipeline."""
    engine = OrderStateMachineExtendedEngine()
    processed_results = []
